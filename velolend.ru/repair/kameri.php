...
<head>
      ...
      <title>Запчасти: камеры</title>
      <meta name="description" content="Продажа велосипедов" />
      <meta name="keywords" content="запчасти, камеры, продажа" />
...
?>
       ...
       <div class="site_content">
        <h2 class="name_page">Камеры для велосипеда</h2>
            <div class="text_description"><p>Велоаптечка заядлого велосипедиста должна содержать массу необходимых в дороге запчастей и инструментов. Но самыми главными являются камеры для велосипеда. Отправляясь в дорогу, не поленитесь взять пару запасных камер, ведь никто не застрахован от таких неприятностей как прокол колеса. При выборе камер для велосипеда, следует обращать внимание на диаметр колеса, материал и производителя, а также тип ниппеля.</p>
            </div>
            <div class="katalog">
                  <table class="table_katalog">
                        <?php 
                         if(isset($_POST['delete']))
                            ...
                            $result = mysqli_query($connection,"SELECT * FROM `products` WHERE `podcategories` = '11';");
                            $i=2;
